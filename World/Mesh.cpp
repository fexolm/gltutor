...
const AttribType g_allAttributeTypes[] =
    {
        ...
        {"half", false, GL_HALF_FLOAT, sizeof(GLhalf), ParseFloats, WriteFloats},
...
struct Attribute {
  Attribute()
      : iAttribIx(0xFFFFFFFF), pAttribType(NULL), iSize(-1), bIsIntegral(false) {}
...
struct MeshData {
  MeshData()
      : oAttribArraysBuffer(0), oIndexBuffer(0), oVAO(0) {}
...
Mesh::Mesh(const std::string &strFilename)
  ...
  {
    const std::string& strDataFilename = strFilename;
