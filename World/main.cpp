...
ProgramData LoadProgram(const std::string &strVertexShader, const std::string &strFragmentShader)
{
  ...
  shaderList.push_back(utils::loadShader(GL_VERTEX_SHADER, strVertexShader));
  shaderList.push_back(utils::loadShader(GL_FRAGMENT_SHADER, strFragmentShader));

  ProgramData data;
  data.theProgram = utils::createProgram(shaderList);
...
{
  UniformColor = LoadProgram("shaders/PosOnlyWorldTransform.vert", "shaders/ColorUniform.frag");
  ObjectColor = LoadProgram("shaders/PosColorWorldTransform.vert", "shaders/ColorPassthrough.frag");
  UniformColorTint = LoadProgram("shaders/PosColorWorldTransform.vert", "shaders/ColorMultUniform.frag");
}
...
void init()
{
  ...
  try
  {
    g_pConeMesh = new Framework::Mesh("meshes/UnitConeTint.xml");
    g_pCylinderMesh = new Framework::Mesh("meshes/UnitCylinderTint.xml");
    g_pCubeTintMesh = new Framework::Mesh("meshes/UnitCubeTint.xml");
    g_pCubeColorMesh = new Framework::Mesh("meshes/UnitCubeColor.xml");
    g_pPlaneMesh = new Framework::Mesh("meshes/UnitPlane.xml");
  }
...
glm::vec3 ResolveCamPosition()
{
  ...
  float phi = utils::DegToRad(g_sphereCamRelPos.x);
  float theta = utils::DegToRad(g_sphereCamRelPos.y + 90.0f);
